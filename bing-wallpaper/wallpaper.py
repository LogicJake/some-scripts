...
def date_plus_day(date, day):
    date = datetime.datetime.strptime(date, "%Y%m%d") + datetime.timedelta(days=day)
...
def download(num_saved, wallpaper_path):
    ...
    try:
        ...
        for i in range(num_wallpaper):
            wallpapers[date_plus_day(content['images'][i]['startdate'], 1)] = bing_prefix + content['images'][i]['url']
